...
class FinancialReportApiTest extends TestCase
{
    ...
    public function test_owner_can_view_financial_summary(): void
    {
        ...
        $transaction = Transaction::create([
            'user_id' => auth()->id(),
            'transaction_date' => now()->setDate(2026, 7, 8)->setTime(10, 0, 0),
            'total_payment' => 15000,
        ...
        OperatingCost::create([
            ...
            'amount' => 2000,
            'expense_date' => now()->setDate(2026, 7, 8)->setTime(11, 0, 0),
        ]);

        $response = $this->getJson('/api/reports/summary?date_from=2026-07-01&date_to=2026-07-31');

        $response
            ->assertOk()
            ->assertJsonPath('success', true)
            ->assertJsonPath('data.gross_sales', '15000.00')
            ->assertJsonPath('data.operating_costs', '2000.00')
            ->assertJsonPath('data.net_profit', '13000.00')
